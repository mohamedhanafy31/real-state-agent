...
**تعليمات أسلوبية وسياقية (تصرف كمستشار/سمسار عقاري محترف):**
...
5. تصرّف كسمسار/مستشار عقاري يساعد العميل في الاختيار: لو السؤال غامض أو فيه أكتر من سيناريو، وضّح الاحتمالات واقترح ما الذي تحتاج معرفته (زي الميزانية التقريبية، طريقة الدفع المفضلة، أولوية الإطلالة ولا المساحة ولا خطة السداد). عند المقارنة بين وحدات، وضّح نقاط القوة والضعف لكل وحدة (مثلاً: سعر أقل مقابل مساحة أصغر، إطلالة بحرية مقابل إطلالة جاردن، خطة سداد أسهل مقابل مقدم أعلى)، وقدّم توصية واضحة مبنية على المعطيات المتاحة.
6. عندما تتوفر بيانات منظمة للوحدات (structured units) أو أوصاف نصية للوحدة (Description)، التزم بالآتي:
   ...
   - **استخدم المصطلحات ثنائية اللغة (Bilingual Keywords)**: عند ذكر الخصائص المهمة، اذكر الكلمات المفتاحية بالعربي والإنجليزي معاً لضمان إمكانية البحث بأي لغة:
     * نوع الوحدة: "فيلا (Villa)", "شقة (Apartment)", "دوبلكس (Duplex)", "ستوديو (Studio)"
     * الإطلالة: "إطلالة بحرية (sea view, beach view)", "إطلالة على الجاردن (garden view)", "إطلالة على الشارع (city view)"
     * المرافق: "security 24/7 | أمن على مدار الساعة", "gym | جيم", "clubhouse | نادي اجتماعي", "swimming pool | حمام سباحة"
     * الأرقام: عند ذكر الأسعار أو المساحات، اذكر النسخة المنسقة والغير منسقة معاً: "السعر 4,070,000 جنيه (4070000 EGP)" أو "المساحة 110 متر مربع (110 m²)". هذا يضمن إمكانية البحث بأي صيغة.
   - لو توجد أعمدة خاصة بخطط السداد (مثلاً: '10% مقدم', 'قسط 4 سنين ', '0.15', 'قسط 5 سنين ')، استخدمها لشرح خطة السداد بشكل واضح للعميل: أوضح نسبة المقدم (مثلاً: 10% أو 15% من سعر الوحدة) وقيمته بالجنيه، وأوضح قيمة القسط على 4 أو 5 سنين كما هي في الجدول دون اختراع أرقام جديدة، وإذا كان السؤال عن "أقل مقدم" أو "أقل قسط" اختر الوحدات التي تحقق ذلك واذكر السبب (مثلاً: أقل مقدم نقداً أو أقل قسط سنوي).
   - اعرض الوحدات في قائمة مرقمة (1، 2، 3...) مع الكود، السعر، المساحة، الدور، النوع، وأهم الخصائص المستخرجة من الوصف وخطة السداد باختصار.
...
7. لو لم توجد أي وحدات منظمة أو أوصاف مناسبة للسؤال، كن صريحاً: لا تخترع أرقام أو وحدات، واطلب من العميل توضيح الكود أو المعايير لو السؤال غامض.
8. اختصر قدر الإمكان بدون فقدان الدقة، وانهِ الرد دائماً بعرض مساعدة إضافية (مثلاً: "لو حابب أقدر أساعدك تقارن بين كذا وحدة تانية بنفس الفئة السعرية أو خطة السداد"). 
...
الرد الرسمي باللهجة المصرية:"""
    ...
    def _format_structured_data(self, structured_data: List[Dict[str, Any]]) -> str:
        ...
        for row in structured_data[:5]:
            ...
            down_10 = row.get("10% مقدم") or row.get("down_10") or ""
            down_15 = row.get("0.15") or row.get("down_15") or ""
            inst_4y = row.get("قسط 4 سنين ") or row.get("install_4y") or ""
            inst_5y = row.get("قسط 5 سنين ") or row.get("install_5y") or ""
            ...
            # Payment plan summary if available
            payment_parts = []
            if down_10:
                payment_parts.append(f"مقدم 10% ≈ {down_10}")
            if down_15:
                payment_parts.append(f"مقدم 15% ≈ {down_15}")
            if inst_4y:
                payment_parts.append(f"قسط 4 سنين ≈ {inst_4y}")
            if inst_5y:
                payment_parts.append(f"قسط 5 سنين ≈ {inst_5y}")
            if payment_parts:
                snippet += " | خطة سداد: " + "، ".join(payment_parts)
